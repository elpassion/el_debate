...
describe Comment do
  ...
  it 'returns default values when comment has not user assigned' do
    expect(comment.user.image_url).to eq '/assets/dashboard/default_user.png'
    expect(comment.user.name).to eq 'Anonymous'
  end

  it 'returns slack user values when comment has user assigned' do
    expect(comment_with_user.user.image_url).to eq slack_user.image_url
    expect(comment_with_user.user.name).to eq slack_user.name
  end
end
